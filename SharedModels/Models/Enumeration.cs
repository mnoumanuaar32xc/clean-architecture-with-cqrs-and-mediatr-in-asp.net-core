...
namespace SharedModels.Models
{
    public abstract class Enumeration : IComparable
    {
        ...
        public static IEnumerable<T> GetAllByEnumValues<T>(IEnumerable<int> enumValues) where T : Enumeration
        {
            return GetAll<T>().Where(item => enumValues.Contains(item.Value));
        }
        ...
        {
            return TryParse(item => item.Name == valueOrName, out enumeration) || int.TryParse(valueOrName, out var value) && TryParse(item => item.Value == value, out enumeration);
        }
        ...
        private static TEnumeration Parse<TEnumeration, TIntOrString>(TIntOrString nameOrValue, string description, Func<TEnumeration, bool> predicate) where TEnumeration : Enumeration
        {
